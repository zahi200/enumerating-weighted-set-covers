...
public class TeamsEnumerationExp6Min {

    public static final int NUM_OF_INTERVALS = 10;

    private LinkedHashSet<Expert> experts; // all available experts
    private Set<Skill> skills;   // elements to cover by the sets

    public TeamsEnumerationExp6Min(LinkedHashSet<Expert> experts, Set<Skill> skills) {
    ...
    public double[] enumerateMinimally(int numOfResults, String experimentName) {
        ...
        double cost;
        int numOfResultsPerInterval = numOfResults/NUM_OF_INTERVALS;
        double[] timeResults = new double[10];
        int index=0;
...
//                printTeam(team);
                countPrintedResult++;
                // measuring the time for 100results
                if(countPrintedResult%numOfResultsPerInterval==0) {
                    end = System.currentTimeMillis();
                    double time100Results = (double)(end - start)/(double)1000;
                    timeResults[index] = time100Results;
                    index++;
